...
class NormalizationTransformer(ConstraintTransformer):
    ...
    def transform_expression_constraint(
        self, constraint: "ExpressionConstraint"
    ) -> "ExpressionConstraint":
        # ExpressionConstraints are returned as-is.
        return constraint

    def transform_comparison_constraint(
        self, constraint: "ComparisonConstraint"
    ) -> "ComparisonConstraint":
        # ComparisonConstraints do not require normalization.
        return constraint

    def transform_forall_constraint(
        self, constraint: "ForallConstraint"
    ) -> "ForallConstraint":
    ...
    def transform_exists_constraint(
        self, constraint: "ExistsConstraint"
    ) -> "ExistsConstraint":
        # Recursively normalize the inner statement.
        normalized_statement = constraint.statement.transform(self)
    ...
    def transform_disjunction_constraint(
        self, constraint: "DisjunctionConstraint"
    ...
    def transform_conjunction_constraint(
        self, constraint: "ConjunctionConstraint"
    ) -> "ConjunctionConstraint":
        # Recursively normalize each conjunct.
        normalized_constraints = [c.transform(self) for c in constraint.constraints]
        return ConjunctionConstraint(constraints=normalized_constraints)

    def transform_implication_constraint(
        self, constraint: "ImplicationConstraint"
    ) -> "DisjunctionConstraint":
